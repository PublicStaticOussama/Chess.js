...
class ChessGame {
    ...
    updateMoveMask(piece_symbol, i, j, mask, ignore_king_mask=false, xray_through_king=false) {
        switch(piece_symbol) {
            case "p":
            case "P":
            ...
            case "k":
            case "K":
                this.updateKingMoveMask(piece_symbol, i, j, mask, !ignore_king_mask)
                break
            case "n":
            case "N":
    ...
    updateMovePossibilities(piece_symbol, i, j, mask, ignore_king_mask=false, xray_through_king=false) {
        this.updateMoveMask(piece_symbol, i, j, mask, ignore_king_mask, xray_through_king) // it updates this.cursor_move_mask
        this.updateUiByMask() // updates UI based on this.cursor_move_mask
        // TODO: request to server; it runs its own this.updateMoveMask() then responds with its own this.cursor_move_mask and replaces the client side mask
        // this.updateUiByMask() // updates again based on this.cursor_move_mask returned by the server
    }
